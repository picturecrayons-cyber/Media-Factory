/**
 * Self-hosted Better Auth for Bridge (server-only).
 * Production uses the canonical BETTER_AUTH_URL. Vercel previews deliberately
 * resolve the app origin from the request so OAuth state cookies and callbacks
 * stay on the same preview host.
 */
...
// BETTER_AUTH_URL here would put the callback on production while the state
// cookie remains on the preview host. Production continues to use the explicit
// canonical URL.
